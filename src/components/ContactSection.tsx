  ...
  <p className="text-sm md:text-base text-foreground/70">Share your goals. We’ll propose a clear, pragmatic path. Email <a href="mailto:naitronsolutions@gmail.com" className="underline underline-offset-4">naitronsolutions@gmail.com</a>.</p>
        <form className="grid gap-4" action={(fd) => startTransition(() => onAction(fd))} suppressHydrationWarning>
          <div className="grid gap-2">
            ...
            <input id="name" name="name" required disabled={isPending} className="h-11 px-3 rounded-md border border-black/25 dark:border-white/20 bg-background/70 backdrop-blur-sm focus:outline-none focus:ring ring-foreground/30" suppressHydrationWarning />
          </div>
          <div className="grid gap-2">
            <label className="text-sm font-medium" htmlFor="email">Email</label>
            <input id="email" type="email" name="email" required disabled={isPending} className="h-11 px-3 rounded-md border border-black/25 dark:border-white/20 bg-background/70 backdrop-blur-sm focus:outline-none focus:ring ring-foreground/30" suppressHydrationWarning />
          </div>
            <input type="text" name="company" className="hidden" tabIndex={-1} autoComplete="off" />
          <div className="grid gap-2">
            <label className="text-sm font-medium" htmlFor="message">Project / Message</label>
            <textarea id="message" name="message" rows={5} required disabled={isPending} className="p-3 rounded-md border border-black/25 dark:border-white/20 bg-background/70 backdrop-blur-sm focus:outline-none focus:ring ring-foreground/30 resize-none" suppressHydrationWarning />
          </div>
